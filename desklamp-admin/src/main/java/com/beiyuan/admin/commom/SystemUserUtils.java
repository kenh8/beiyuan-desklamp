package com.beiyuan.admin.commom;

import com.alibaba.fastjson.JSON;

import com.beiyuan.admin.rpc.pms.SysResourceApiConsumer;
...
public class SystemUserUtils {
    ...
    public static SysUserDto getCurrentUser(){
    	...
    	if(redisService.mapExists(AdminRedisConstants.ADMIN_SECURITY_USER_MAP_KEY, userName)){
    		user=redisService.getHashStringObject(AdminRedisConstants.ADMIN_SECURITY_USER_MAP_KEY, userName, SysUserDto.class);
    	}
    	if(null != user){
	...
	public static void initUserLoginSuccessEvent(SysUserDto user){
        redisService.setHashStringObject(AdminRedisConstants.ADMIN_SECURITY_USER_MAP_KEY,user.getUsername(), user, 0);
