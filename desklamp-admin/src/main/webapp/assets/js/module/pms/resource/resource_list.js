...
function renderOperate(json,htmlObj){
    ...
    _delbtn.attr("href",_rootpath+"/pms/resource/delete?id="+json.id+"&callbackMethod=ajaxTreeDone");
...
function operateRender(value, row, index){
    var action= '<a data-toggle="modal" data-target="#base_modal" href="'+rootPath+'/pms/resource/edit?id='+row.id+'"  class="view-link">编辑</a>' +
        ' <a href="javascript:void(0);" onclick="ajaxDel(\''+rootPath+'/pms/resource/delete?modifyType=delete&id='+row.id+'\')" class="view-link">删除</a>';
