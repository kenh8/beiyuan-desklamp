package com.beiyuan.base.util.excel.excel;
...
public class ExcelExportUtil {
	 ...
	 * @return
	 */
	public static int setRowData(Workbook workBook, Sheet sheet,int rowIndex,String[] properties,List<Object> data,boolean isMap){
		int i = rowIndex ;
	...
	public static void readInRows(Workbook workBook,String[] properties,String[] headers,List data,boolean isMap){
		try {
			...
			if(data != null && data.size() > 0){
				setRowData(workBook, sheet,1, properties, data,isMap);
			}
		} catch (Exception e) {
		...
		for(int i = 0; i < data.size(); i++) {
			...
			try {
				...
				if (data.get(i) != null && data.get(i).size() > 0) {
					...
					setHeadRow(workBook, sheet, headers);
					setRowData(workBook, sheet, 1, properties, (List<Object>) data.get(i).get("data"), isMap);
				}
			} catch (Exception e) {
				logger.error("--" + e.getMessage());
			}
