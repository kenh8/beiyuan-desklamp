...
import com.beiyuan.cache.utils.ProtostuffSerializer;
...
public class RedisService {
    ...
    public <T> T getHashStringObject(String mapKey, String field,Class<T> targetClass){
        ShardedJedis shardedJedis = null;
        try {
            shardedJedis = shardedJedisPool.getResource();
            if(shardedJedis.exists(mapKey)){
                String obj = shardedJedis.hget(mapKey, field);
                if(!StringUtils.isEmpty(obj)){
                    return JSON.parseObject(obj, targetClass);
                }
            }
        } catch (Exception e) {
            log.error("",e);
    ...
    public <T> void setHashStringObject(String mapKey, String fieldKey,T value, int cacheSeconds){
        ShardedJedis shardedJedis = null;
        try {
            shardedJedis = shardedJedisPool.getResource();
            shardedJedis.hset(mapKey, fieldKey, JSON.toJSONString(value));
            if(cacheSeconds != 0){
                shardedJedis.expire(mapKey, cacheSeconds);
            }
        } catch (Exception e) {
