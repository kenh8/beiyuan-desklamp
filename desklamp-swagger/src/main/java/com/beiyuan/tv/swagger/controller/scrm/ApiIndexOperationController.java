 ...
 */
@Api(value = "运营活动",tags = "运营活动模块相关")

@Controller
@RequestMapping("/swagger/api/tv/v1.0/operation/*")
public class ApiIndexOperationController extends BaseController {

    @ApiOperation(value = "获取运营的活动,同时只能有一个活动处于有效状态")
    ...
    @RequestMapping(value = "getIndexOperation", method = RequestMethod.GET)
    public @ResponseBody
    Object getIntegral(){

        String resp = OkHttp3Utils.getInstance().doPost(Global.getServiceUrl() + HttpApiConstant.GET_INDEX_OPERATION, new
                HashedMap());
        return resp;
    }
